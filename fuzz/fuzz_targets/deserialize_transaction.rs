...
fn do_test(data: &[u8]) {
    let tx_result: Result<litcoinlib::blockdata::transaction::Transaction, _> = litcoinlib::consensus::encode::deserialize(data);
    match tx_result {
        Err(_) => {},
        Ok(mut tx) => {
            let ser = litcoinlib::consensus::encode::serialize(&tx);
            assert_eq!(&ser[..], data);
            let len = ser.len();
            let calculated_weight = tx.get_weight();
            for input in &mut tx.input {
                input.witness = litcoinlib::blockdata::witness::Witness::default();
            }
            let no_witness_len = litcoinlib::consensus::encode::serialize(&tx).len();
