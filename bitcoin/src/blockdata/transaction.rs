...
impl OutPoint {
    ...
    ///
    /// ```rust
    /// use litecoinlib::constants::genesis_block;
...
impl<E> EncodeSigningDataResult<E> {
    ...
    ///
    /// ```rust
    /// # use litecoinlib::consensus::deserialize;
    /// # use litecoinlib::Transaction;
    /// # use litecoinlib::crypto::sighash::{LegacySighash, SighashCache};
    /// # use bitcoin_hashes::{Hash, hex::FromHex};
    /// # let mut writer = LegacySighash::engine();
    /// # let input_index = 0;
    /// # let script_pubkey = litecoinlib::ScriptBuf::new();
